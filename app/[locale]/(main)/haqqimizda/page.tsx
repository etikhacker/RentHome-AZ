...
export default function HaqqimizdaPage() {
  return (
    <>
      <SiteHeader />
      <div className="max-w-[720px] mx-auto px-4 sm:px-7 py-10 sm:py-14">
        <h1 className="font-display text-xl sm:text-2xl font-medium mb-2">Necə işləyir</h1>
        <p className="text-sm text-ink-soft mb-8 sm:mb-10">
          RentHome AZ Azərbaycanda ev icarəsini vasitəçisiz, sadə və etibarlı edir.
        </p>

        <div className="space-y-4 sm:space-y-6">
          {steps.map((s) => (
            <div key={s.title} className="bg-paper border border-line rounded-2xl p-4 sm:p-5">
              <h2 className="font-display text-base sm:text-lg font-medium mb-1.5">{s.title}</h2>
